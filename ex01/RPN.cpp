...
/*   Updated: 2024/07/11 18:14:35 by agengemb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void RPN::resolve(char c)
{
	...
	if (mstack.size() < 2)
		throw (std::invalid_argument(std::string("Missing operands")));
	b = mstack.top();
	mstack.pop();
	a = mstack.top();
	mstack.pop();
...
RPN::RPN(std::string& input)
{
	...
	while (temp[i] != '\0')
	{
		if (temp[i] >= '0' && temp[i] <= '9')
			...
			throw(std::invalid_argument(std::string("Bad input")));
		++i;
	}
	if (mstack.size() != 1)
		throw (std::invalid_argument(std::string("Missing operators")));
}
